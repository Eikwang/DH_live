...
def get_scheduler(optimizer, niter, niter_decay, lr_policy='cosine', lr_decay_iters=50):
    ...
    if lr_policy == 'lambda':
        ...
        def lambda_rule(epoch):
            ...
            elif epoch <= fixed_end_epoch:
                return 1.0

            else:
                decay_epoch = epoch - fixed_end_epoch
                if decay_epoch >= niter_decay:
                    return 0.005 

                ratio = decay_epoch / niter_decay
                cosine_decay = 0.5 * (1 + math.cos(math.pi * ratio)) 

                return 0.005 + 0.995 * cosine_decay
