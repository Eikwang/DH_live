...
def main():
    ...
    # 获取模型名称参数，如果没有提供则使用默认值
    if len(sys.argv) == 5:
        model_name = sys.argv[4]
        model_path = f"checkpoint/{model_name}"
    else:
        model_path = "checkpoint/render.pth"
    print(f"Model path is set to: {model_path}")

    audioModel = AudioModel()
    audioModel.loadModel("checkpoint/audio.pkl")

    renderModel = RenderModel()
    renderModel.loadModel(model_path)
    ...
    os.system(
        "ffmpeg -y -i {} -i {} -c:v copy -pix_fmt yuv420p -loglevel quiet {}".format(save_path, wavpath, output_video_name))
